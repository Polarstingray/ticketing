              ...
              <Link key={t.id} to={`/tickets/${t.id}`} className={styles.rowLink}>
                ...
                >
                  ...
                  <div className={styles.rowSide}>
                    ...
                    />
                    {/* Archiving is the tail end of a ticket's life, so the
                        shortcut only shows where it makes sense: a closed
                        ticket that isn't archived yet. */}
                    {t.status === "closed" && !t.archived && (
                      <button
                        type="button"
                        className={styles.archiveBtn}
                        disabled={busyId === t.id}
                        aria-label={`Archive ticket #${t.id}`}
                        onClick={(e) => {
                          // The row is a <Link>; don't navigate on our click.
                          e.preventDefault();
                          e.stopPropagation();
                          archive(t.id);
                        }}
