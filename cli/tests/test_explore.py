...
def test_blocks_come_from_the_commit_not_the_worktree(git_repo, commit):
    ...
    blocks = explore.build_code_blocks_for_feature(git_repo, ["a.py"], sha)
...
def test_blocks_read_the_worktree_when_there_is_no_rev(git_repo, commit):
    ...
    (git_repo / "a.py").write_text("dirty\n", encoding="utf-8")
    blocks = explore.build_code_blocks_for_feature(git_repo, ["a.py"], None)
    assert blocks[0]["content"] == "dirty"


def test_hallucinated_paths_are_skipped(git_repo, commit):
    sha = commit("init", {"a.py": "x\n"})
    blocks = explore.build_code_blocks_for_feature(git_repo, ["nope.py", "a.py"], sha)
    assert [b["filename"] for b in blocks] == ["a.py"]


def test_only_the_first_few_files_are_quoted(git_repo, commit):
    files = {f"f{i}.py": "x\n" for i in range(6)}
    sha = commit("init", files)
    blocks = explore.build_code_blocks_for_feature(git_repo, sorted(files), sha)
    assert len(blocks) == explore.MAX_FILES_PER_FEATURE
